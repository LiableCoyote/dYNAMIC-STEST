...
4. `docs/planning/A_engine_build_scaffolding.md` — build/CI/naming/asset scaffolding (Area A, done).
...
- **Areas D–M:** 🔲 not started. **D** = PSOE faction semantics (Caballerista/Prietista/Besteirista/UGT); **E** = party-landscape content; **F** = the four net-new subsystems (anarchism, regional autonomy, agrarian, church/army-Africa); **G** = policy-card content; **H** = the event corpus (the bulk); **I** = advisors; **J** = qdisplay/UI; **K** = assets; **L** = localization; **M** = balancing.

## How to continue (recommended next step)

The **electoral engine is mechanically complete and playable in isolation**, but the
game is **not end-to-end playable** yet — it still boots into Weimar narrative content.
...
2. **Area D (PSOE factions)** — smaller, self-contained; remaps the six-faction model
   and the `ideology.scene.dry` deck to PSOE's real factional geometry. Good if you
   want another bounded engine-ish pass before the large content investment.
