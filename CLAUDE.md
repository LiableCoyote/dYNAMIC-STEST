...
9. `docs/planning/A_engine_build_scaffolding.md` — build/CI/naming/asset scaffolding (Area A, done).
...
**The game is end-to-end playable and the dead German corpus is gone** (April 1931 →
three elections → July 1936 coup → one of four Spanish endings, tree ~75% smaller with
no known German content on any confirmed-reachable path). What remains is content depth
and a couple of flagged loose ends:
...
1. **Area G (policy-card content)** — the ~37 `government_affairs` cards beyond the ones
   ...
   framing (e.g. `economic_policy.scene.dry`, explicitly gated off rather than surfaced
   broken). A bounded, well-precedented pass.
