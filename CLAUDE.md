...
8. `docs/planning/A_engine_build_scaffolding.md` — build/CI/naming/asset scaffolding (Area A, done).
...
- **Area H Phase 1 (the playable spine):** ✅ done — **the game is now end-to-end playable, April 1931 → July 1936.** Built the three gaps that were blocking this: (1) `events/coalition_formation.scene.dry` (new) — the coalition-formation writer, the linchpin Area F/C left waiting; after each of the three elections it now actually changes the government (Republican-Socialist → Radical-CEDA *bienio negro* → Popular Front, with the real Prieto/Caballero split modeled as a genuine choice), writing `chancellor`/ministries/the coalition flags every time. (2) `election_1928.scene.dry`'s post-election flow rebuilt: the live Area-C seat math (preserved intact) now leads into a Spanish results screen (H-2) instead of a ~2200-line dead German coalition-menu tree (left orphaned, flagged inline). (3) The July-1936 endgame: 4 new escalation events (`asturias_rising`, `popular_front_victory_shock`, `spring_1936_breakdown`, `calvo_sotelo_assassination`) plus `events/july_1936_coup.scene.dry` (trigger + resolver, computing forces entirely from live militia/army vars) sets one of `republic_victory`/`long_war`/`total_defeat`; `game_over.scene.dry`'s endings were fixed to match — including the single highest-visibility bug found in this pass, `@no_hitler` firing on *every* Spanish playthrough regardless of outcome (its condition was trivially always true), now repurposed as the coup-averted ending. Also: gated off 9 events H-0's recon flagged as reachable-but-German (all turned out to be already-dead via a different mechanism, gated anyway for clarity); fixed the turn-1 opening prose (epigraph, difficulty screen, dropped the obsolete Mod Loader paragraph); neutralized `post_event.scene.dry`'s two engine-scale dead blocks Area F flagged (force computation, coalition taxonomy) that had been producing `NaN` every turn since Area B. Verified with a standalone Node **end-to-end playthrough simulation** (boot → 3 elections → escalation chain → coup → ending, zero unexpected `NaN`), plus per-stage simulations, grep/compiled-output sweeps, and headless load checks. **Found but explicitly out of scope, flagged in `H_event_corpus.md`:** a second dead-German block interleaved inside `@post_election_1928`'s live seat math (hardcoded old party-letter coalition percentages, distinct from the two `post_event.scene.dry` blocks); `ending_slides.scene.dry` (the optional "View ending slides" screen) is still fully unconverted German/Austria/Mussolini flavor content; the ~196 dead German files, the 72-file `reichswehr_*→army_*` rename, and deep flavor chains remain deferred to Area H Phase 2 / Area G.
- **Areas G, I–M:** 🔲 not started. **G** = policy-card content (the ~37 `government_affairs` cards beyond what Areas B/E/F/H touched — the Economic Policy hub deck is explicitly gated off pending this, per Area H's finding); **I** = advisors; **J** = qdisplay/UI (already has flagged findings waiting: the `nsdap_r`-class dead qdisplay ids and the `hindenburg_angry` id used across 83 files); **K** = assets; **L** = localization; **M** = balancing. Area H itself has a Phase 2 (deferred bulk deletion of dead German files, the `reichswehr_*` rename, deep flavor chains, `ending_slides.scene.dry`'s conversion) — see `H_event_corpus.md`'s Deferred section.

## How to continue (recommended next step)

**The game is end-to-end playable now** (April 1931 → three elections → July 1936 coup →
one of four Spanish endings), but two categories of debt remain, both already scoped:

1. **Area G (policy-card content)** — the ~37 `government_affairs` cards beyond the ones
   Areas B/E/F/H already touched still reference pre-rename variables and German policy
   framing (e.g. `economic_policy.scene.dry`, explicitly gated off in Area H rather than
   surfaced broken). A bounded, well-precedented pass.
2. **Area H Phase 2** — the deferred bulk work `H_event_corpus.md` lists: deleting the
   ~196 dead German files, the 72-file `reichswehr_*→army_*` rename, `ending_slides.scene.dry`'s
   conversion, and the small interleaved dead block inside `@post_election_1928` found during
   H-6. None of this blocks playability; it's cleanup and depth.
