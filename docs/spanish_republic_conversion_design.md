...
### J. Quality Displays & UI Text — *Size M · Partial rewrite* — ✅ done
...
- **✅ done** — see `docs/planning/J_qdisplays_ui.md` for full detail. The set is now 22→12 files, all generic or Spanish, zero German names: renamed the misnamed-but-live `hindenburg_angry`→`president_approval`, added a `coup_readiness` display, and deleted 11 dead/orphaned German defs (`schleicher_*`, `nazi_funds`, `hindenburg_*`, `cvp_dnvp_balance`, `camarilla_strength`). **Reachability of the two H2-flagged status screens was fully determined** from the hardcoded `index.html` tab bar (rather than left uncertain): the "Emergency Status" tab is `display:none`/dead (deleted the `@emergency` sub-scene), while "Politics", "Polls", and "Scheming" are live/visible. Converted the live German content: the Politics tab's German-splinter Party-Leadership/Industrial-Backing rosters → a Spanish 7-party roster; the Polls tab's German demographic table → the live Spanish class×party matrix; and the "Scheming" presidential camarilla → the **Spanish 1936 military conspiracy** (Mola/Sanjurjo/Franco/Goded/Queipo + Calvo Sotelo/Falange/Requetés), driven by live Area-F coup vars and retitled "Conspiracy". **Found but out of scope:** `library.scene.dry`'s `@government`/`@weimar_timeline` are still German narrative prose (a content-area follow-up, not J's qdisplay/UI lane); the Conspiracy page's general portraits are text-only pending an Area-K asset pass.
